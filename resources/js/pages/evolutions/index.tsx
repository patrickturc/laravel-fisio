...
export default function EvolutionsIndex({ evolutions = [] }: { evolutions: any[] }) {
    const [search, setSearch] = useState('');
    
    const filteredEvolutions = evolutions.filter(e => 
        e.patient?.name?.toLowerCase().includes(search.toLowerCase()) || 
        e.queixa_principal?.toLowerCase().includes(search.toLowerCase())
    );
                                    ...
                                    >
                                        ...
                                        <div className="flex-shrink-0 flex items-center md:flex-col justify-between md:justify-center gap-3">
                                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                <span>Profissional: {evo.profissional_id?.substring(0,8)}</span>
                                            </div>
